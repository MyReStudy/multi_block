...
from draw import draw_deep, draw
...
def generate_data(require_info):
...
for i in range(0, 1000):
    ...
    info_stsp = generate_data(require_info)  # 随机抽取的点的idx(pad)、坐标(pad)、坐标(没有pad)
    ...
    time3 = time.time()
    length, x, y = tsp_solver(model, info_stsp, file)
    time4 = time.time()
...
# 保存到Excel文件
df.to_excel(f'am_exact_aco_{file}.xlsx', index=False)
